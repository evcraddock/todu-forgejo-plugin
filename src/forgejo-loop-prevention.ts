export interface ForgejoWriteRecord {
  key: string;
  timestamp: string;
}

export interface ForgejoLoopPreventionStore {
  ...
  listAll(): ForgejoWriteRecord[];
}

const DEFAULT_MAX_AGE_MS = 10 * 60 * 1000;

export function createForgejoLoopPreventionStore(): ForgejoLoopPreventionStore {
  ...
  return {
    ...
    listAll(): ForgejoWriteRecord[] {
      const records: ForgejoWriteRecord[] = [];
      for (const [key, timestamp] of writes.entries()) {
...
export function createForgejoWriteKey(
